# Binance perpetual USDT kline

`GET /api/v1/crypto/binance/perp/usdt/kline`

**Timestamp Rule**: query window is in UTC Unix seconds. Bar boundaries align to UTC (00:00 UTC for `1d`+). A bar is only returned if the query range fully contains `[time_open, time_close]`. The quote currency is fixed at USDT in the URL path — pass only the base token symbol (`BTC`, not `BTCUSDT`). Returns the perpetual futures contract; for funding rate / open interest / long-short ratio see the other endpoints in this skill.
...
| Open price | `price_open` | number | Opening mark price (USDT) |
...
| Close price | `price_close` | number | Closing mark price (USDT) |
